function write_TB_obs_Ruralizate_atms( time,num_lat,num_lon,obs_point,chan,input_dir,output_dir,scene_name )
%edit by hongpengfei
%2018.11.29

tb=zeros(obs_point,chan);
for i=1:chan
    TB_dir=sprintf('%s/%s',input_dir,time);
    TB_file=sprintf('%s/Typhoon%s_%s_%s_%s_C%s_H.mat',TB_dir,scene_name,time(7:8),time(9:10),time(11:12),num2str(i));
    if(exist(TB_file,'file')==2)
        load(TB_file);
        TbMap=interpolation(TbMap,num_lat,num_lon);
