...
input_dir='/g3/hanwei/hpf/DOTLRT_TB/maria';     %输入正演亮温位置
% input_meteo_dir='D:\科研使我快乐\科研\maliya\大气参数';      %写观测资料必要的大气参数变量。
% output_dir='D:\\科研使我快乐\\科研\\maliya\\DOTLRT_obs_file\\TB';     %写观测资料的输出文件夹
output_dir='/g3/hanwei/hpf/DOTLRT_obs_file/maria/TB';     %写观测资料的输出文件夹
input_all_the_fiel_dir='/g3/hanwei/hpf/DOTLRT_TB/maria/all_the_same_file';    %写观测资料必要的大气共有的大气参数变量。
...
num_lat=251;      %仿真网格大小
num_lon=251;      
chan=42;           %仿真总的频点个数（仿真的最大通道数），如果有些频点没有模拟出亮温，那该频点亮温全部复制为零。
...
zsfile=sprintf('%s//zs.txt',input_all_the_fiel_dir);
GRAPESzs=load(zsfile);
GRAPESzs=GRAPESzs(1:251,501:751);
GRAPESzs=repmat(reshape(GRAPESzs',obs_point,1),1,chan);
%处理地表高度信息
%打开文件
...
%        write_TA_obs(time,num_lat,num_lon,obs_point,chan,input_dir,output_dir,scene_name);
    end
end
