function TB = BG_deconv(TA,pattern)
%   函数功能： 对没有观测噪声的亮温图像TA进行BG解卷积处理，提升图像分辨率和精度****************
%              返回BG法重建后的亮温图像
%  
%   输入参数:
%    TA           ：待重建的输入亮温图像TA
...
parfor row_number = 1+n:row-n
    for col_number = 1+n:col-n        
        ...
        q = pinv(G)*(v+((1-u'*pinv(G)*v)/(u'*pinv(G)*u))*u);                                          %计算权值q
%    q = G.^(-1)*(v+((1-u'*G.^(-1)*v)/(u'*G.^(-1)*u))*u); 
%    q = pinv(R)*((v*cos(r)+(1-u'*pinv(R)*v*cos(r))*u)/(u'*pinv(R)*u));
%    q = pinv(R)*(v*cos(r)+(1-u'*pinv(R)*v*cos(r))*u/(u'*pinv(R)*u));
%    q = R.^(-1)*(v*cos(r)+(1-u'*R.^(-1)*v*cos(r))*u/(u'*R.^(-1)*u));
