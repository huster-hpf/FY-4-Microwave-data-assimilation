...
observation_path = 'bufr';   %观测数据存储目录
file_path = 'bak';   %正演仿真亮温文件存储目录
satellite_payload = 'AMSUA';                         %设置载荷名称
orbit_height = 824;                                 %设置载荷轨道高度, unit:km  ATMS：824km ， 风云三号：836km ?????AMSUA??????
observation_time = '2016-9-13-5：4：49-5：12：48'; %载荷观测亮温数据时间
scene_time = '2016-09-13-05：00：00';               %场景正演亮温数据时间
background_time = '2016-09-13-05：00：00';          %场景插值亮温数据时间
channel_start_index = 1;                            %设置亮温数据的频率通道编号起始
channel_end_index = 15;                             %设置亮温数据的频率通道编号终止 ATMS-22； FY3C_MWTS-13； FY3C-MWHTS-15；FY3B-MWTS-4；FY3B-MWHS-5 amsua-15 mhs-5
RT_model = 'CRTM';                                 %可以设置为‘DOTLRT’或'RTTOV'
%设置所有频率通道的仿真结果保存列表
...
for freq_channel = 1:15 %循环仿真每个频点的观测亮温                           
    ...
    if flag_L2 ==1
       TB_filename =  sprintf('%s-L2-C%d_%s',satellite_payload,freq_channel,scene_time);%正演亮温数据文件名
...
%        end
       ...
       TB_filename =  sprintf('C%d_%s',freq_channel,scene_time);%正演亮温数据文件名
       TB_matfile = sprintf('%s\\%s.mat', file_path,TB_filename);
       load(TB_matfile);
       TB=TbMap;
%        if freq_channel==1 || freq_channel==2
...
%% 以下是每个频点都不一样的辐射计载荷参数
    if (strcmpi('ATMS',satellite_payload)) 
       ...
       if (strcmpi('AMSUA',satellite_payload))
        switch freq_channel%中心频率,     %带宽, 单位:Hz       %噪声系数,单位:dB   %天线口径，单位：米
            case 1, freq= 23.8e9;   bandwith = 270e6;    noise_figure= 8.5;antenna_diameter = 0.275; integral_time = 165*10^(-3);   
            case 2, freq= 31.4e9;   bandwith = 180e6;    noise_figure= 7.5;  antenna_diameter = 0.215; integral_time = 165*10^(-3);   
            case 3, freq= 50.3e9;   bandwith = 180e6;    noise_figure= 8.5;  antenna_diameter = 0.125; integral_time = 158*10^(-3);   
            case 4, freq= 52.8e9;   bandwith = 400e6;    noise_figure= 8.5;  antenna_diameter = 0.125; integral_time = 158*10^(-3);   
            case 5, freq= 53.596e9; bandwith = 2*170e6;  noise_figure=8;  antenna_diameter = 0.125; integral_time = 158*10^(-3);   
            case 6, freq= 54.4e9;   bandwith = 400e6;    noise_figure= 8.5;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 7, freq= 54.94e9;  bandwith = 400e6;    noise_figure= 8.5;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 8, freq= 55.5e9;   bandwith = 330e6;    noise_figure= 8;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 9, freq= 57.29e9;  bandwith = 330e6;    noise_figure= 8;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 10,freq= 57.507e9; bandwith =2*78e6;    noise_figure= 8.5;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 11,freq= 57.66e9;  bandwith = 2*36e6;   noise_figure= 7;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 12,freq= 57.63e9;  bandwith = 2*16e6;   noise_figure= 7;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 13,freq= 57.62e9;  bandwith = 2*8e6;    noise_figure= 6.5;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 14,freq= 57.617e9; bandwith = 2*3e6;    noise_figure= 6;  antenna_diameter = 0.125; integral_time = 158*10^(-3); 
            case 15,freq= 89e9;     bandwith = 6000e6;   noise_figure= 17;  antenna_diameter = 0.075; integral_time = 158*10^(-3); 
        end
       end
    if (strcmpi('MHS',satellite_payload))
        switch freq_channel%中心频率,     %带宽, 单位:Hz       %噪声系数,单位:dB   %天线口径，单位：米
            case 1, freq= 89e9;     bandwith = 2800e6;    noise_figure= 13;antenna_diameter = 0.11; integral_time = 19*10^(-3);   
            case 2, freq= 157e9;    bandwith = 2800e6;    noise_figure= 13;  antenna_diameter = 0.11; integral_time = 19*10^(-3);   
            case 3, freq= 184.31e9; bandwith = 2*500e6;   noise_figure= 12;  antenna_diameter = 0.11; integral_time = 19*10^(-3);   
            case 4, freq= 186.31e9; bandwith = 2*1000e6;  noise_figure= 12;  antenna_diameter = 0.11; integral_time = 19*10^(-3);   
            case 5, freq= 190.31e9; bandwith = 2200e6;    noise_figure= 12;  antenna_diameter = 0.11; integral_time = 19*10^(-3);   
        end
    end
...
%    integral_time = 18*10^(-3);                                 %积分时间, unit:S          ATMS=18ms; AMSUA 1~2=165ms;3~15=158ms                                 
...
%保存单个频率通道的仿真结果%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if flag_save == 1
        ...
        save(['F:\张兰\结果\ATMS-TyphoonMeranti_20160913\ATMS模拟亮温\' MatFileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','ground_resolution','OA_RMSE','OA_Corr_coe','OB_RMSE','OB_Corr_coe');
        save(['F:\张兰\结果\ATMS-TyphoonMeranti_20160913\ATMS模拟亮温\' MatFileName],'TA_observation','Long_observation','Lat_observation','TA_Background','OB_mean','OB_std','OA_mean','OA_std','-append');
        if  flag_Resolution_Enhancement == 1;
            save(['F:\张兰\结果\ATMS-TyphoonMeranti_20160913\ATMS模拟亮温\' MatFileName],'TA_RE','OA_RMSE_RE','OA_Corr_coe_RE','factor_opt','-append'); 
        end
    end
...
if flag_save == 1;
   ...
   save(['F:\张兰\结果\ATMS-TyphoonMeranti_20160913\ATMS模拟亮温\' ListMatFileName],'POES_result_list','POES_PayloadSpec_list');   
end
%part5：end**************************************************************************************************************************************************
