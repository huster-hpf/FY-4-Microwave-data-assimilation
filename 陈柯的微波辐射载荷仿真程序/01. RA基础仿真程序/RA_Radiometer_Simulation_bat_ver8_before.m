...
tic;
%%***************************以下部分是设置标志位和轨道参数***************************************************************************************************
flag_draw_TB = 1;                               %画出输入亮温TB 
flag_draw_TA = 1;                               %画出观测亮温TA 
...
file_path = 'C:\Users\国子监祭酒\Desktop\亮温和坐标文件\静轨\hongxia';   %文件存储目录
file_path1='C:\Users\国子监祭酒\Desktop\亮温和坐标文件\静轨';
%计算场景在经度和纬度方向的观测角范围
...
% scene_name = 'TbMap-50.3-2012-10-29_23_45_00';
...
dir_files = dir([file_path,'\*.mat']);
num_files = length(dir_files);


% for freq_index = channel_start_index:channel_end_index    
for file_index = 1:num_files
    TB_filename = dir_files(file_index).name;    
    TB_matfile = sprintf('%s\\%s', file_path,TB_filename);
    load(TB_matfile);   TB=(TbMap(:,:,1));
        
    tempstr=textscan(TB_filename,'%s', 'delimiter', '-');
    tempstr=tempstr{1,1};
    str_freq=tempstr{2,1};   
    scene_name = [tempstr{size(tempstr,1)-2,1},'-',tempstr{size(tempstr,1)-1,1},'-',tempstr{size(tempstr,1),1}];
    Payload_name = 'GEM5';
    switch str_freq
%      case '50.3',     freq_index = 1; 
%      case '51.76',    freq_index = 2; 
%      case '52.8',     freq_index = 3; 
     case '53.596',   freq_index = 5;          
     case '54.4',     freq_index = 6; 
     case '54.94',    freq_index = 7; 
     case '55.5',     freq_index = 8; 
     case '57.29',     freq_index = 9; 
%      case '88.2',     freq_index = 14;
%      case '119.55',   freq_index = 18; 
%      case '119.85',   freq_index = 19; 
%      case '121.75',   freq_index = 21;          
%      case '123.75',   freq_index = 22;
%      case '165.5',    freq_index = 23; 
%      case '184.31',   freq_index = 28; 
%      case '186.31',   freq_index = 26;          
%      case '187.81',   freq_index = 25;
%      case '190.31',   freq_index = 24;
%      case '381.697',  freq_index = 41; 
%      case '384.197',  freq_index = 40; 
%      case '389.197',  freq_index = 39;          
%      case '398.197',  freq_index = 38;
%      case '425.763',  freq_index = 32; 
%      case '426.263',  freq_index = 31; 
%      case '428.763',  freq_index = 30;             
    end
...
%        save(['..\05. RA仿真结果\' MatFileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','Corr_coefficient','ground_resolution');
       save(['D:\matlab\地球静止轨道模拟亮温\红霞CRTM200_200_10km\模拟CRTM正演亮温\' FileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','Corr_coefficient','ground_resolution');
       if  flag_Resolution_Enhancement == 1;
%            save(['..\05. RA仿真结果\' MatFileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append'); 
           save(['D:\matlab\地球静止轨道正演转BUFR\红霞CRTM200_200_10km\模拟CRTM正演亮温\' FileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append');
       end
    end   
   %给所有channel仿真结果列表赋值
