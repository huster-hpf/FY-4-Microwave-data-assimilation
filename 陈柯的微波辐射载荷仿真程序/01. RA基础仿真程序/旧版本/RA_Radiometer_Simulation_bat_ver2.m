...
file_path = 'D:\当前的工作\2015.03.21 基于资料同化的GEO仿真研究\01. 仿真亮温数据\2016.10.25 强对流天气亮温-北京-GFS-20120721\亮温图像';   %文件存储目录
%计算场景在经度和纬度方向的观测角范围
...
scene_name = 'RainstormBeijing_21_18';
% 设置要批量仿真的通道编号
for freq_index = 1:8    
...
for i = 1:num_r                                                     %对N_r个r值进行试探
    ...
    [RMSE_array(i),~] = Root_Mean_Square_Error(TB,TA_real_BG,RMSE_offset,0);
...
xlabel('经度方向'); ylabel('纬度方向');title(['无噪BG解卷积后的亮温图像@Ch.',num2str(freq_index)]);colorbar;
...
xlabel('经度方向'); ylabel('纬度方向');title(['有噪BG解卷积后的亮温图像@Ch.',num2str(freq_index)]);colorbar;
%part3：end*****************************************************************************************************************************************
