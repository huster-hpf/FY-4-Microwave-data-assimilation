function [ TA ] = conv_PolarOrbit( TB,SateZenith_angle_TA_array,SateZenith_angle_TB_array,Ba,q,factor)
%CONV_ANGLE 包含角度修正的卷积函数
%
[length,TA_num_Latitude,TA_num_Longitude] = size(SateZenith_angle_TA_array);                                                           %TA坐标
[length,num_Latitude,num_Longitude] = size(SateZenith_angle_TB_array);                                                                 %TB坐标
...
% matlabpool open 3;
% parfor num_sample_Longitude=1:TA_num_Longitude
%      for num_sample_Latitude=1:TA_num_Latitude 
for num_sample_Latitude=100
    for num_sample_Longitude=100
pattern = zeros(num_Latitude,num_Longitude);                                                                %初始化方向图
theta_1 = imag(SateZenith_angle_TA_array(num_sample_Longitude,num_sample_Latitude,num_sample_Longitude))*pi/180;                                %指定经纬度的卫星扫描角度
phi_1 = real(SateZenith_angle_TA_array(num_sample_Longitude,num_sample_Latitude,num_sample_Longitude))*pi/180;                          
...
%             Xs = height_orbit*b;
            ...
            theta_2 = imag(SateZenith_angle_TB_array(num_sample_Longitude,num_row ,num_col))*pi/180;                                   %指定经纬度的卫星扫描角度
            phi_2 = real(SateZenith_angle_TB_array(num_sample_Longitude,num_row ,num_col))*pi/180;                                  
