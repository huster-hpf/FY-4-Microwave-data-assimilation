...
satellite_longtitude = -99.5;                   %静止轨道卫星所处经度
satellite_latititude = 0;                       %静止轨道卫星所处纬度
%*********************************************************************************************************
...
load(coordinate_filename); %读取图像对应的经纬度XLAT,XLON
...
InclineAngle_LONG = max(max(XLO))-min(min(XLO));                                                    % Coordinate_LongTA坐标以观测角为基准。此处以经纬度为基准。
InclineAngle_LAT = max(max(XLA))-min(min(XLA));
Coordinate_XLO = linspace(min(min(XLO)),max(max(XLO)),N_Long*sample_factor);       %经度角度坐标向量
Coordinate_XLA = linspace(min(min(XLA)),max(max(XLA)),N_Lat*sample_factor);        %纬度角度坐标向量
[Coordinate_XLO,Coordinate_XLA] =  meshgrid(Coordinate_XLO,Coordinate_XLA); 
Coordinate_XLO = Coordinate_XLO';
Coordinate_XLA = Coordinate_XLA';
[theta,phi] = SatelliteAngleCaculate( Coordinate_XLO,Coordinate_XLA,satellite_longtitude,satellite_latititude,R,orbit_height );       %theta为经度lon方向观测角，phi为纬度lat方向观测角
SateZenith_angle_TA = theta + 1i*phi;
[theta_TB,phi_TB] = SatelliteAngleCaculate(XLO,XLA,satellite_longtitude,satellite_latititude,R,orbit_height );                        %theta为经度lon方向观测角，phi为纬度lat方向观测角
...
patternarray_path = 'F:\11.14_pattern_correct\pattern_array';   %文件存储目录
load(sprintf('%s\\Patternarray_C%d_q%d.mat',patternarray_path,freq_index,illumination_taper));%载入一维方向图pattern_array
...
TA_noiseless = conv_angle(TB,Fov_LongTA,Fov_LatTA,Fov_Long,Fov_Lat,SateZenith_angle_TA,SateZenith_angle_TB,Ba,illumination_taper,orbit_height,factor);
% TA_noiseless = conv_angle_bat2(TB,Fov_LongTA,Fov_LatTA,Fov_Long,Fov_Lat,SateZenith_angle,Ba,illumination_taper,orbit_height,factor,pattern_array);
% %直接使用卷积函数计算观测亮温
