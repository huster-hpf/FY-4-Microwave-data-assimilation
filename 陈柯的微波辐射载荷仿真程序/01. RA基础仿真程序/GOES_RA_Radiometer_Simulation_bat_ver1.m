%本程序模拟静止轨道实孔径辐射计载荷观测亮温——模拟真实观测角度
...
file_path = 'C:\Users\ai\Desktop\TBTEST';  %文件存储目录
file_path1='C:\Users\ai\Desktop\TBTEST\latlon';
%设置仿真场景和频率通道
scene_time='2018-07-08_12_00_00';
scene_name = 'noise_maliya070812';
Payload_name = 'DOTLRT';
channel_start_index = 1;
channel_end_index = 29;
...
% coordinate_filename2 = sprintf('%s\\lon.mat', file_path);
load(coordinate_filename);
% load(coordinate_filename2);
Long_scene =XLO.';  Lat_scene = XLA.';  %计算场景格点经纬度坐标
[N_Lat,N_Long] = size(Long_scene);
...
%      [N_Lat1,N_Long1] = size(TB); 
     ...
     if  flag_draw_TB == 1;
         ...
         xlabel('经度方向'); ylabel('纬度方向');title('正演亮温图像TB');colorbar;
     end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 if flag_save == 1
       ...
       save(['..\taili5db\' MatFileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','RMSE_antenna','RMSE_noise','Corr_coefficient','ground_resolution'); 
       if  flag_Resolution_Enhancement == 1;
...
if flag_save == 1;
   SaveFileName = sprintf('GOES-MW_%s_C%d_%s',Payload_name,freq_index,scene_name);
   if flag_Resolution_Enhancement == 1;
   SaveFileName = sprintf('%s_%s', SaveFileName,Resolution_Enhancement_name);
   end
   SaveMatFileName = sprintf('%s_result_list.mat', SaveFileName);
   save(['..\05. RA仿真结果\' SaveMatFileName],'RMSE_list','RMSE_antenna_list','RMSE_noise_list','Corr_coefficient_list','NEDT_list','HPBW_list','ground_resolution_list','SLL_list','MBE_list');
   if  flag_Resolution_Enhancement == 1;
