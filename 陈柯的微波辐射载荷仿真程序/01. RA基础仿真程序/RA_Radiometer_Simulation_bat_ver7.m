...
file_path = 'F:\read_innnotb_out';   %文件存储目录
%计算场景在经度和纬度方向的观测角范围
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if flag_save == 1
       ...
       save(['F:\张兰\结果\静止轨道\sandy\sandy观测亮温\' MatFileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','RMSE_antenna','RMSE_noise','Corr_coefficient','ground_resolution'); 
       if  flag_Resolution_Enhancement == 1;
           save(['F:\张兰\结果\静止轨道\sandy\sandy观测亮温\' MatFileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append');
           if  flag_PDF_calc == 1;
               save(['F:\张兰\结果\静止轨道\sandy\sandy观测亮温\' MatFileName],'TA_PDF_RE','HPBW_RE','ground_resolution_RE','SLL_RE','MBE_RE','-append'); 
           end
       end
...
disp([Payload_name,'载荷对',scene_name,'的仿真结果']);
...
if  flag_Resolution_Enhancement == 1;
    ...
    disp('--------------------------------------------------------------------------')
    if  flag_PDF_calc == 1;
        disp('分辨率增强处理后的3dB波束宽度  地面分辨率  副瓣电平 主波束效率 ');
...
if flag_save == 1;
   ...
   save(['F:\张兰\结果\静止轨道\sandy\sandy观测亮温\' SaveMatFileName],'RMSE_list','RMSE_antenna_list','RMSE_noise_list','Corr_coefficient_list','NEDT_list','HPBW_list','ground_resolution_list','SLL_list','MBE_list');
   if  flag_Resolution_Enhancement == 1;
   save(['F:\张兰\结果\静止轨道\sandy\sandy观测亮温\' SaveMatFileName],'RMSE_RE_list','Corr_coefficient_RE_list','factor_opt_list','-append'); 
       if  flag_PDF_calc == 1;
            save(['F:\张兰\结果\静止轨道\sandy\sandy观测亮温\' SaveMatFileName],'HPBW_RE_list','ground_resolution_RE_list','SLL_RE_list','MBE_RE_list','-append'); 
       end
   end    
end
toc;