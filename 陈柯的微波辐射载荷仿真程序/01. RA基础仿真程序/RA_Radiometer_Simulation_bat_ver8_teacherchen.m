...
file_path = 'C:\Users\国子监祭酒\Desktop\亮温和坐标文件\2017080406';   %文件存储目录
file_path1='C:\Users\国子监祭酒\Desktop\亮温和坐标文件';
%计算场景在经度和纬度方向的观测角范围
...
dir_files = dir([file_path,'\*.mat']);   %dir函数的作用:返回文件夹中的所有文件或者文件夹所组成的列表
num_files = length(dir_files);


% for freq_index = channel_start_index:channel_end_index    
for file_index = 1:num_files
    TB_filename = dir_files(file_index).name;    %输出文件名
    TB_matfile = sprintf('%s\\%s', file_path,TB_filename);  %输出文件名
    load(TB_matfile);   TB=(TbMap(:,:,1));   %3维矩阵
        
    tempstr=textscan(TB_filename,'%s', 'delimiter', '-');   %读取文件的一些信息
    tempstr=tempstr{1,1};        
    str_freq=tempstr{2,1};     %频率？   $这三句没看懂！！！
    scene_name = [tempstr{size(tempstr,1)-2,1},'-',tempstr{size(tempstr,1)-1,1},'-',tempstr{size(tempstr,1),1}];
    Payload_name = 'GEM5';
    switch str_freq
     case '23.8V GHz_2015',     freq_index = 44;
     case '31.4',     freq_index = 45
     case '50.3',     freq_index = 1; 
     case '51.76',    freq_index = 2; 
     case '52.8',     freq_index = 3; 
     case '53.596',   freq_index = 4;          
     case '54.4',     freq_index = 5; 
     case '54.94',    freq_index = 6; 
     case '55.5',     freq_index = 7;  
     case '57.29',    freq_index =8;    
    ...
    [Fov_Long,Fov_Lat] = meshgrid(Coordinate_Long,Coordinate_Lat);          %经度纬度方向二维坐标矩阵   meshgrid用于从数组a和b产生网格。生成的网格矩阵A和B大小是相同的
    %画出输入亮温图像
    if  flag_draw_TB == 1;   %imagesc是软件Matlab中的一个函数。imagesc(A) 将矩阵A中的元素数值按大小转化为不同颜色
...
%        save(['..\05. RA仿真结果\' MatFileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','Corr_coefficient','ground_resolution');
       save(['F:\模拟观测亮温\奥鹿300_300剩余亮温\模拟观测亮温\' FileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','Corr_coefficient','ground_resolution');
       if  flag_Resolution_Enhancement == 1;
%            save(['..\05. RA仿真结果\' MatFileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append'); 
           save(['F:\模拟观测亮温\奥鹿300_300剩余亮温\模拟观测亮温\' FileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append');
       end
    end   
   %给所有channel仿真结果列表赋值
