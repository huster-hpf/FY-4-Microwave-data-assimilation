%本程序模拟整个圆盘实孔径辐射计载荷观测亮温——模拟真实观测角度
%by 洪鹏飞 2018.07.30 

clear;
close all;
%%***************************以下部分是设置标志位和轨道参数**********************************************************
flag_draw_TB=0;                                 %画原始亮温TB标志位
...
file_path = 'D:\matlab\全圆盘数据';   %文件存储目录


%计算场景的经度和纬度坐标,然后根据卫星轨道转换为相对卫星的角度坐标
all_filename = sprintf('%s\\GIMS_TB_50.3_183-1_Simu.mat', file_path);
load(all_filename);
ETA=reshape(eta,1074,1074);
XI=reshape(xi,1074,1074);
num_TB=size(TB,2);
% TB_angle=reshape(TB(:,1),1074,1074);
% [LAT,LON]=eta_xi_to_lat_lon(TB_angle,ETA,XI,R,orbit_height,Long_Subsatellite,Lat_Subsatellite);
% [THETA_Long,FAI_Lat]=lat_lon_to_coordinate_long_coordinate_lat(TB_angle,LAT,LON,R,orbit_height,Long_Subsatellite,Lat_Subsatellite);
% angle_Long=max(max(THETA_Long));
% angle_Lat=max(max(FAI_Lat));
angle_Long_max=8.6177;
angle_Long_min=-8.6111;
angle_Lat_max=8.6177;
angle_Lat_min=-8.6111;
angle_Long=angle_Long_max-angle_Long_min;
angle_Lat=angle_Lat_max-angle_Lat_min;



for TB_index = 1:num_TB
    Tb_channel=reshape(TB(:,TB_index),1074,1074);
    switch TB_index       %%%选择通道
        case 1,      freq_index = 1;
        case 2,      freq_index = 2;
%         case 3      freq_index = 3;
%         case 4      freq_index = 4;
...
%         case 8      freq_index = 8;
%         case 9      freq_index = 9;
%         case 10     freq_index = 10;
%         case 11     freq_index = 11;
%         case 12     freq_index = 12;
%         case 13     freq_index = 13;
%         case 14     freq_index = 14;
%         case 15     freq_index = 15;
%         case 16     freq_index = 16;
%         case 17     freq_index = 17;
%         case 18     freq_index = 18;
    end
    
    T_max=max(max(Tb_channel)); T_min=min(min(Tb_channel));
    [N_Lat,N_Long] = size(Tb_channel);
    %计算图像空间角度坐标参数*********************************************
    %计算空间格点大小
    ...
    %画出输入亮温图像
    if  flag_draw_TB == 1;
        figure;imagesc(Coordinate_Long,Coordinate_Lat,Tb_channel,[T_min T_max]);axis equal;xlim([-angle_Long/2,angle_Long/2]);ylim([-angle_Lat/2,angle_Lat/2]);
...
%% 以下是每个频点需要修改的辐射计参数
    switch freq_index   %中心频率,单位:Hz  %带宽, 单位:Hz     %噪声系数,单位:dB   %天线口径，单位：米
             ...
             case 5, freq= 53.596e9; bandwith = 400e6;    noise_figure= 5;  antenna_diameter = 5;                     
             case 6, freq= 54.4e9;   bandwith = 400e6;    noise_figure= 5;  antenna_diameter = 5;                     
             case 7, freq= 54.94e9;  bandwith = 400e6;    noise_figure= 5;  antenna_diameter = 5;                      
             case 8, freq= 55.5e9;   bandwith = 330e6;    noise_figure= 5;  antenna_diameter = 5;                     
             case 9, freq= 57.29e9;  bandwith = 330e6;    noise_figure= 5;  antenna_diameter = 5; 
             case 33,freq= 57.507e9; bandwith = 2*78e6;   noise_figure= 5;  antenna_diameter = 5; 
...
%模拟天线扫描方式获得观测亮温TA
    TA_noiseless = Antenna_conv(Tb_channel,angle_Lat,angle_Long,sample_width_Long,sample_width_Lat,N_Long,N_Lat,Ba,illumination_taper);
...
%        save(['..\05. RA仿真结果\' MatFileName],'TA_noiseless','TA','Antenna_Pattern','NEDT','HPBW','SLL','MBE','RMSE','Corr_coefficient','ground_resolution');
       save(['D:\matlab\quanyuanpan\' FileName],'TA_noiseless','TA');
       if  flag_Resolution_Enhancement == 1;
%            save(['..\05. RA仿真结果\' MatFileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append'); 
           save(['D:\matlab\taili\temp\out_noise\' FileName],'TA_RE','RMSE_RE','Corr_coefficient_RE','factor_opt','-append');
       end
    end   
   %给所有channel仿真结果列表赋值
...
%    save(['..\05. RA仿真结果\' SaveMatFileName],'RMSE_list','Corr_coefficient_list','NEDT_list','HPBW_list','ground_resolution_list','SLL_list','MBE_list');
%    if  flag_Resolution_Enhancement == 1;
%    save(['..\05. RA仿真结果\' SaveMatFileName],'RMSE_RE_list','Corr_coefficient_RE_list','factor_opt_list','-append'); 
%    end    
% end
toc;
